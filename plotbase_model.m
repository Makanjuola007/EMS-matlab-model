...
%% Pricing model
grid_price_per_kWh = 0.15;  % $ per kWh (standard rate)
price_signal = grid_price_per_kWh * grid_available; % 0 when OFF, standard when ON

% Instantaneous cost in $
instant_cost = (meter_power/1000) .* price_signal;  % (kW × $/kWh) = $/hour
cumulative_cost = cumtrapz(time, instant_cost)/3600; % $ over time
