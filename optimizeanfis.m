...
% === ANFIS TRAINING SECTION ===

% Define inputs and target
X = [bsoc' load_power' ac_power' inverter_temp'];  % Inputs
Y = total_energy_gen';                             % Target (kWh)

trainingData = [X Y];

% Generate and train ANFIS
fis = genfis1(trainingData, 3);
[trainedFIS, trainError] = anfis(trainingData, fis, 50);

% Save trained FIS
writeFIS(trainedFIS, 'anfis_energy_optimizer');
disp('✅ ANFIS training complete. FIS saved as "anfis_energy_optimizer.fis"');

% Automatically add ANFIS block to Simulink model
anfisBlock = [modelName '/ANFIS_EnergyOptimizer'];
add_block('fuzzylogic/Fuzzy Logic Controller', anfisBlock, 'Position', [x+100, 100, x+250, 200]);

% Connect input signals to ANFIS block
inputSignals = {'Bsoc', 'loadpower', 'AC_ouputPOWER', 'inveterTemp'};
for i = 1:length(inputSignals)
    inputLine = [modelName '/' inputSignals{i} '_From'];
    add_line(modelName, [inputSignals{i} '_From/1'], ['ANFIS_EnergyOptimizer/' num2str(i)]);
end

% Connect ANFIS output to Scope
add_block('simulink/Sinks/Scope', [modelName '/ANFIS_Output_Scope'], 'Position', [x+300, 130, x+370, 160]);
add_line(modelName, 'ANFIS_EnergyOptimizer/1', 'ANFIS_Output_Scope/1');
